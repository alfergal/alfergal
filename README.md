<h1 align="center">Alberto Fernández Gálvez</h1>

<p align="center">
  <strong>Data Scientist · Senior Software Developer</strong><br>
  Machine Learning · Generative AI · Software Engineering
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/alberto-fernandez-galvez/">LinkedIn</a>
  ·
  <a href="mailto:albertofer1997@gmail.com">Email</a>
  ·
  <a href="https://github.com/alfergal">GitHub</a>
</p>

---

<table>
<tr>
<td width="50%" valign="top">

<pre>
                ...               ...             
                   ...::---=+-::..   .            
              ..--=*+*###%%%@@@%%#+-. ..          
             . -%####%%@@%%@@@@%%@@%*:  .         
            . :*%*#%@%%%%%%@@%%%%%@@@@*: .        
           ...+#*%#+====*%%###%@@@@@@@@%- .       
           . =@%#+:......:----=+#%@@@@@@%: .      
           . *%+:   ....  ..:-::-=+*#@@@@- .      
          . .*-::::-+**+++=--=::::--=+#@@- .      
          ..:--+##:---=*###*=:-++#**+++#%...      
            :++.--.-=+###*-+*+*%%%%#%%#*=         
          ..-+:.:-...:-==-:+==#%###*+*#+=:.       
       . .=-::...::......:=:.:*#++**#++=##:..     
         .::=.... ..::---:: .:=*=-::=+=+=. .      
        . .:-.....  ..:-:=+--++++=--=++=  .       
          ...:...........:-=+#*+---==++...        
           ..:.......:::::-=-+*+=---==. .         
             ::.....:-----==+**+==-=+. .          
         .....::..::....:---=+#+-==+: .           
  .......    ..:-::.:::-=+**+====+=. ....         
         ..:-...:==:.....::=====**.       ...     
 ...::--=++**:...:--===----=++**+##*+=-..     ..  
.--===*+##**+:......:-++++****+=+@@@%%%#*+-::.    
.-=-+**=+#*#=..........:-+++=--=#@%###%##**++=:.. 
.:=--*#+=+*#+-....:..::-=+=---=+%%####%#*+===--:. 
.:-=-=#+==+#*+=::..:::-=+=--==+#%####%*#*+++=-:.. 
 .:---+*===+**++=-..--==---+*##%#*#%%**#+++--::.  
 ..:---+=-=+=+*+++++++++++*####**###++*++=---:..  
  ...:.:----=-=+===++++******++**++=-=---:::..    
      ...:::::::------==+===--=---::::......      
</pre>

</td>
<td width="50%" valign="top">

<pre>
alfergal@github
────────────────────────────────────────

Role       Data Scientist
           Senior Software Developer

Experience 5+ years building
           production software

Focus      Machine Learning
           Generative AI
           Predictive Analytics
           Software Architecture

Languages  Python · TypeScript
           JavaScript · PHP
           Java · SQL · Bash

Data / AI  scikit-learn · Pandas
           NumPy · TensorFlow
           Keras · MLflow · PySpark

Software   Angular · Drupal
           Spring Boot · StencilJS
           REST APIs

DevOps     Docker · Git · Jenkins
           CI/CD · Composer

Education  Applied Data Science · UOC
           Web Application Development
</pre>

</td>
</tr>
</table>

## About

I'm a **Data Scientist and Senior Software Developer** with **5+ years of professional software-development experience**, combining production engineering with machine learning, predictive analytics and generative AI.

My background spans the full software lifecycle: from building data pipelines and predictive models with **Python, Pandas, SQL and scikit-learn**, to designing scalable frontend architectures with **Angular and TypeScript**, developing Drupal/PHP solutions, integrating APIs, automating tests and working with CI/CD environments.

I enjoy working at the intersection of **software engineering and data**: turning ideas and models into maintainable products that can actually be shipped.

## Professional highlights

- Built **chatbots, AI agents and automation solutions** with Python and LLMs.
- Developed **data-preparation pipelines and predictive models** with Python, Pandas, SQL and scikit-learn.
- Led frontend architecture for applications using **Angular 17+, TypeScript, JavaScript and PHP**.
- Worked with **Docker, Jenkins, Git, Composer and MySQL** in CI/CD environments.
- Implemented automated testing with **BackstopJS, Behat and PHPUnit**.
- Developed backend services with **Java and Spring Boot** in microservice architectures.

## Core stack

```text
Data & AI       Python · scikit-learn · Pandas · NumPy · TensorFlow · Keras · MLflow · PySpark
Frontend        Angular · TypeScript · JavaScript · HTML5 · CSS3 · Tailwind CSS · StencilJS
Backend         PHP · Drupal · Java · Spring Boot · REST APIs · SQL · MySQL
DevOps          Docker · Git · Jenkins · Composer · Linux
Testing         Behat · PHPUnit · BackstopJS
```

## Education

**BSc in Applied Data Science**  
Universitat Oberta de Catalunya — 2023–2026

**Higher Diploma in Web Application Development**  
IES Tetuán — 2018–2020

## Certifications

- **Machine Learning Specialization** — Stanford University / DeepLearning.AI, 2025
- **Deep Learning Specialization** — Stanford University / DeepLearning.AI, 2025
- **IBM Data Science Professional Certificate** — IBM, 2025

---
